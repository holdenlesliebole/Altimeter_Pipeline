...
% SIO uses sequential anchoring (not survey) because the Scripps Submarine
% Canyon creates complex bathymetry that the MOP transect doesn't capture.
% The survey profile at X_shore=267m shows -4m depth, but the instrument
% is at -6m — survey anchoring would introduce a ~2m offset.
% FIXED 2026-08-24. pvuLabel is the PUV L2/L3 FILE name ("SIO_6m"); the
% site_grain_size table is keyed on SITE ("MOP511_6m"). build_L4_site used pvuLabel
% for both, so the SIO grain-size lookup threw and it silently fell back to
% ks = 10*D50 with a default D50 = 250 um: ks = 2.5 mm against the correct
% 2.5*D84 = 0.90 mm, a 2.8x roughness overestimate, and a different roughness
% formulation rather than a degraded version of the right one. The two labels are now
% separate parameters. At Torrey and Solana they happen to coincide.
L4_SIO = build_L4_site(L3root, pvuRoot, 'SouthSIOPier', ...
    'depths', 6, ...
    'pvuLabel', "SIO_6m", ...
    'grainSizeLabel', "MOP511_6m", ...
    'pvuDeployments', {'SIO24A','SIO24B','SIO24C','SIO25A','SIO25B','SIO25C','SIO25D','SIO25E'}, ...
...
for d = 1:numel(tp_depths)
    ...
    try
        L4 = build_L4_site(L3root, pvuRoot, 'TorreyPines', ...
            ...
            'pvuDeployments', {'TBR23','TOR23W','TOR24S','TOR24W','TOR25S'}, ...
            'anchorMethod', 'survey', ...
...
% Survey-anchored like Torrey. Explicit PUV list (SOL only) prevents the
% cross-site mismatch that previously sent Solana bursts to a non-Solana
% deployment (ISSUE-001). Instrument position = MOP654.5 7m.
fprintf('\n========================================\n');
fprintf('Solana Beach MOP654 7m\n');
fprintf('========================================\n');
try
    L4_SOL = build_L4_site(L3root, pvuRoot, 'SolanaBeach', ...
        'depths', 7, ...
        'pvuLabel', "MOP654_7m", ...
        'pvuDeployments', {'SOL23','SOL24','SOL25A','SOL25B'}, ...
        'anchorMethod', 'survey', ...
        'instrumentLat', 32.99064, 'instrumentLon', -117.27897, ...
        'mopStation', "D0654", ...
        'savePath', fullfile(outDir, 'L4_SOL_7m.mat'));
catch ME
    fprintf('  FAILED: %s\n', ME.message);
end
