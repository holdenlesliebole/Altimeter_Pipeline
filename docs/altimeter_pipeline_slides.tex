...
\begin{frame}{Remaining Work}
  \begin{enumerate}
    \item \textbf{Storm event decomposition}: Dec 28, 2023 case study at
          TP multi-depth --- simultaneous $H_s$, $\tau_b$, and $dz/dt$.
    \item \textbf{Equilibrium model critique}: compute Dean
          disequilibrium, test whether $dz/dt \propto$ disequilibrium.
          The $n \approx 0$ exponent suggests this will fail.
    \item \textbf{Hoefel \& Elgar deep dive}: quantify the
          $\langle u|u|^2 \rangle$ correlation at TP 5m, compare
          with their Duck94 results.
    \item \textbf{Site-specific $D_{50}$}: lab grain size analysis will
          sharpen all Shields comparisons.
    \item \textbf{Time-varying doffp}: feed altimeter bed level back to
          PUV for improved bed velocity estimates.
  \end{enumerate}
\end{frame}
