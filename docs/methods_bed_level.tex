...
acoustic return) and values exceeding \SI{5}{\meter} (catching firmware
Error-3 artefacts) were flagged and set to NaN. Quality flags were
stored as a uint16 bitmask: bit~1 for invalid/zero values, bit~2 for
phase-space spikes.

For instruments operating in burst mode (e.g., 60~samples at
\SI{2}{\hertz} every 20~minutes), the phase-space despike was applied
to each physical burst independently rather than to the full time
series with interpolated inter-burst gaps. Burst boundaries were
auto-detected from time gaps exceeding \SI{10}{\second}. This prevents
artefacts from interpolating across the \SIrange{10}{20}{\minute}
gaps between bursts, which the phase-space algorithm would otherwise
misidentify as spikes or real transitions.
...
A second phase-space despike was then applied to the burst-median
altitude time series itself, run iteratively (up to 5~passes) until
convergence. This catches entire corrupted bursts---caused by firmware
errors, acoustic interference, or biological targets---that survived
the per-ping despike because the burst median was biased by corrupted
samples. For the most heavily affected deployment (Torrey Pines 5m,
where firmware Error-3 messages corrupted $\sim$30\% of bursts),
the burst-level despike removed 1472~additional spikes across 5~passes,
preserving the underlying morphological signal including a
\SI{600}{\milli\meter} storm accretion event.
...
sensor-to-bed distance) is positive; erosion is negative. This
convention correctly describes storm-driven offshore sand deposition
as accretion at the subaqueous measurement point, even when the same
storm causes erosion of the subaerial beach.
...
profiles. The feasibility of inverting the acoustic backscatter for
suspended sediment concentration (SSC) was assessed by comparing
water column backscatter intensity during calm and energetic conditions
(classified by $|d\Delta z/dt|$). No detectable correlation was found:
the ratio of energetic to calm backscatter in the water column
($0$--$\SI{0.3}{\meter}$ from sensor) was $1.00 \pm 0.01$ for both
burst-mean and burst-maximum profiles. The \SI{450}{\kilo\hertz}
EA400 backscatter is therefore used only for bed detection and
qualitative visualization, not for quantitative SSC estimation.
...
\subsubsection{Cross-deployment baseline anchoring}

Consecutive instrument deployments at the same location produce
independent relative bed level records, each with an arbitrary
baseline at the first valid burst. Two methods were used to create
continuous time series across deployment boundaries:

\paragraph{Survey anchoring.} For sites with subaqueous survey
coverage (Torrey Pines, Solana Beach), each deployment was shifted
to match the NAVD88 elevation measured by the nearest jetski GPS
survey. The per-deployment offset was computed from:
%
\begin{equation}
  z_\text{offset} = z_\text{survey} + a_\text{match} / 1000
\end{equation}
%
where $z_\text{survey}$ is the survey elevation (m~NAVD88) at the
instrument's cross-shore position and $a_\text{match}$ is the
burst-median altitude (mm) at the time nearest the survey.
Deployments without a direct survey match ($> \SI{48}{\hour}$) were
interpolated from neighboring deployments. At Torrey Pines MOP~586,
survey-anchored records at \SI{7}{\meter}, \SI{10}{\meter}, and
\SI{15}{\meter} depth showed good agreement with independent survey
elevations throughout the record. The \SI{5}{\meter} location showed
discrepancies of $\sim \SI{0.7}{\meter}$ during April 2024, coinciding
with a period of firmware data corruption. At Solana Beach, physical
relocation of the echosounder pipe during maintenance (re-jetting)
introduced spatial offsets between deployments that the survey
anchoring could not fully reconcile.

\paragraph{Sequential anchoring.} For SIO~pier, consecutive deployments
were chained by matching the last valid burst of deployment $N$ to the
first valid burst of deployment $N{+}1$. This is valid because
instrument swaps occurred on the same dive day, with the pipe mount
point unchanged. Sequential offsets were typically
$< \SI{30}{\milli\meter}$, confirming the physical consistency of the
mount across swaps. The absolute bed elevation was computed from the
known sensor height above the bed at installation
($z_\text{sensor} = \SI{-4.96}{\meter}$~NAVD88).

Note that although jetski GPS surveys were available at MOP~511 (14
surveys reaching $-\SI{11.5}{\meter}$), survey anchoring was not
applied at SIO because the instrument is located near the head of
Scripps Submarine Canyon, where the local bathymetry deviates
significantly from the MOP transect profile. The survey profile at
the instrument's cross-shore position (X$_\text{shore}$ =
\SI{267}{\meter}) shows $\sim \SI{-4}{\meter}$~NAVD88, while the
instrument is at $\sim \SI{-6}{\meter}$~NAVD88---a \SI{2}{\meter}
discrepancy caused by canyon-related bathymetric complexity.

\subsection{Level 4: Wave forcing integration}

Burst-averaged bed level products (L3) were merged with co-located
PUV spectral wave products (L2/L3 from the PUV Pipeline) and CDIP
MOP spectral wave model data to create an L4 dataset with
simultaneous wave forcing and bed response.

\subsubsection{PUV timestamp matching}

For each altimeter burst timestamp, the nearest PUV L2 spectral
segment within $\pm \SI{5}{\minute}$ was identified by
nearest-neighbor matching. The $\pm \SI{5}{\minute}$ tolerance is
appropriate because both PUV segments (\SI{17}{\minute} averages)
and altimeter bursts (\SIrange{2.5}{15}{\minute} medians) are
already integrated quantities; interpolation between consecutive
averages would not add physical information. The median timestamp
offset between matched pairs was \SI{2.5}{\minute}.

The L4 struct uses altimeter timestamps as the backbone (response
variable), with PUV fields set to NaN where no match exists (PUV
instrument failures, deployment gaps).

\subsubsection{MOP wave model gap-filling}

Continuous hourly wave parameters ($H_s$, $T_p$) from the CDIP
Monitoring and Prediction (MOP) spectral wave model were loaded for
each site's MOP station and interpolated to the burst timestamps.
This fills all PUV gaps with modeled wave data, providing 100\%
wave height coverage over the full altimeter record. The data source
is tracked per burst ($H_s$ from PUV or MOP) to distinguish
in-situ measurements from model output. MOP wave parameters do not
include near-bed quantities (orbital velocity, shear stress, velocity
moments), which are only available from PUV in-situ measurements.

L4 products were generated for SIO~pier at \SI{6}{\meter} and Torrey
Pines at \SI{5}{\meter}, \SI{10}{\meter}, and \SI{15}{\meter}, with
PUV coverage of 17--35\% and MOP gap-fill to 100\% $H_s$ coverage.
