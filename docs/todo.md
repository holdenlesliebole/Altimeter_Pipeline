...
- [x] **Continuous near-bed orbital velocity / bed stress (Ub gap-fill)** — DONE 2026-05-22. `build_L4_site` now builds `Ub_combined`/`Ub_source` + `tau_b_combined`/`shields_combined`: measured SS-band (0.04-0.25 Hz) Ub where PUV exists, else a spectral linear-theory Ub from the MOP frequency spectrum (shoaled to instrument depth), per-site calibrated on the PUV overlap. Validated across all PUV deployments: own-spectrum slope~1.0/R²~0.99; MOP-driven R²~0.7-0.9 (weakest at SIO/canyon). tau_b via Soulsby f_w.
- [ ] **(Follow-on) Continuous velocity nonlinearity from MOP — skewness/asymmetry via Ursell number (Ruessink et al. 2012).** Governs transport DIRECTION (the depth-partitioned erosion/accretion of Paper 1), which Ub/shields don't capture. Reconstruct Su/Au from MOP (Hs, peak k, depth) → Ursell → Ruessink Boltzmann parameterization; **validate against measured PUV `L2.vmom.skewness`/`asymmetry`** (same approach as the Ub validation). Expect moderate skill (R²~0.5-0.7; nonlinearity is harder than Ub) and strongest relevance at shallow depths (5-7 m, higher Ur); small/noisy at 10-15 m. If it validates, add `Su_combined`/`Au_combined` to L4; if not, keep measured-only during PUV windows. Worth a validation pass — low risk, directionality is core to the A-direction story.
