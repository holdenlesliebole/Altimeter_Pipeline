function cfg = TP24_config()
    ...
    cfg.deployments(k).tz_offset_hours  = [8 0];   % file1 local PST (+8), file2 UTC (0); per ISSUE-001 verified map
    ...
    cfg.deployments(k).echosounderFiles = {};
    cfg.deployments(k).tz_offset_hours  = 0;   % 2024+ altimeter clock = UTC (per ISSUE-001 verified map)
